...
namespace DST.Common.Helper
{
    ...
    internal class StringConverter : TypeConverterBase
    {
        public override object Convert(Type desiredType, Type inputType, object input, out bool conversionSucceeded)
        {
            ...
            if (input == null || (input.GetType() == typeof(string) && ((String)input) == String.Empty))
            {
                return input;
            }
        ...
        {
            return IsTypeConvertible(desiredType, inputType, input, out exactMatch) && desiredType == typeof(String);
        }
    }
}