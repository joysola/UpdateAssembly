...
namespace TileImageViewer
{
    ...
    public static class FontImages
    {
        ...
        static FontImages()
        {
            ...
            {
                var fs = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("TileImageViewer.IconFont.FontAwesome.ttf");
                ...
                if (m_fontsElegant.Count <= 0)
                {
                    lock (m_fontsElegant)
                    {
                        if (m_fontsElegant.Count <= 0)
                        {
                            ...
                            if (!File.Exists(strFile))
                            {
                                var fs = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("TileImageViewer.IconFont.ElegantIcons.ttf");
                                FileStream sw = new FileStream(strFile, FileMode.Create, FileAccess.Write);
                                fs.CopyTo(sw);
                                sw.Close();
                                fs.Close();
