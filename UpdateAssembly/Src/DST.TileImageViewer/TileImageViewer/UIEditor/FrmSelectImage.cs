...
namespace TileImageViewer
{
    public partial class FrmSelectImage : Form
    {
        ...
        {
            string[] nameList = System.Enum.GetNames(typeof(TileImageViewer.FontIcons));
            var lst = nameList.ToList();
            lst.Sort();
            foreach (var item in lst)
            {
                TileImageViewer.FontIcons icon = (TileImageViewer.FontIcons)Enum.Parse(typeof(TileImageViewer.FontIcons), item);
                Label lbl = new Label();
                lbl.AutoSize = false;
                ...
                lbl.Text = "       " + item;
                lbl.Image = TileImageViewer.FontImages.GetImage(icon, 32, Color.FromArgb(255, 77, 59));
        ...
        private void lbl_DoubleClick(object sender, EventArgs e)
        {
            ...
            TileImageViewer.FontIcons icon = (TileImageViewer.FontIcons)lbl.Tag;
            int intSize = ucTextBoxEx1.InputText.ToInt();
            if (intSize <= 0)
                intSize = 32;
            SelectImage = TileImageViewer.FontImages.GetImage(icon, intSize, txtForeColor.BackColor, txtBackcolor.BackColor == Color.White ? Color.Empty : txtBackcolor.BackColor);
